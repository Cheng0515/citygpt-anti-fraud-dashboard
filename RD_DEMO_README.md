...
2. 使用統計
   - 日 / 週 / 月 KPI、AI 使用量（Token）、個人月用量監控、部門與角色用量、常被引用文件、找不到答案與低相關度問題。
3. 稽核日誌
   - 明確定義成功 / 失敗與稽核範圍；提供操作者、時間、操作類型、模組、動作、結果、原因、Trace ID、資料快照與 Token 用量異常通知；標示至少保留 180 天。
4. 回饋管理
   - 填寫回饋、1-10 分回饋評分、文字意見 200 字上限、正向回饋率目標 70%、指派、標籤、狀態、處理說明。
5. 系統狀態
   - 以「AI 回答可用狀態」呈現可引用、需要處理與可能影響回答的文件；技術處理狀態、失敗原因與重新執行放在次要層級。RAG 文件來源、權限與上架狀態只作為輔助資訊唯讀顯示。
...
4. 切到「使用統計」，切換日 / 週 / 月，確認 Token、個人月用量、部門/角色彙整、引用文件與問題清單。
5. 切到「稽核日誌」，先確認成功 / 失敗定義與稽核事項，再查看人員 / 時間 / 操作類型篩選、事件原因、Trace ID、資料快照、敏感/異常事件與 Token 用量異常通知。
6. 切到「回饋管理」，調整 1-10 分評分並將一筆回饋改成已完成。
7. 切到「系統狀態」，先看哪些文件可能影響 AI 回答，再查看建議處理、同步 / 讀取 / 搜尋資料狀態；需處理文件可按重新執行。
...
- 成功：權限與系統規則通過，要求的動作已完成；不代表 AI 回答內容一定正確。
- 失敗：操作未完成或被 SSO、權限、安全規則或系統錯誤擋下；不代表使用者惡意。
- 稽核登入與帳號事件、管理操作、下載 / 匯出，以及敏感、異常、Token 超標或服務錯誤。
