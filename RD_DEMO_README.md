...
2. 使用統計
   - 日 / 月 / 年切換，可分別選擇開始與結束日期、月份或年度；顯示總提問數、使用人數、正向回饋率、每人每月用量前 10 名、部門提問次數前 10 名與 KMS 引用文件排名。
...
4. 回饋管理
   - 管理使用者針對每段 AI 回答提交的 1-10 分評分與文字留言；顯示使用者、部門、代理人、原始問題、被評分回答段落及回饋內容，並提供指派、標籤、處理狀態與管理者備註。
...
6. 切到「回饋管理」，確認使用者、部門、代理人、分數、留言與被評分回答段落，再將一筆回饋改成已完成。
