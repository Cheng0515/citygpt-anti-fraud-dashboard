...
describe('CityGPT admin demo', () => {
  ...
  it('shows completed-period usage rankings and keeps monitoring alerts separate', () => {
    ...
    expect(screen.getByRole('button', { name: /上一日/ })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /上週/ })).toBeInTheDocument()
    ...
    expect(screen.getByText('每人每月用量排名')).toBeInTheDocument()
    expect(screen.getByText('部門提問次數排名')).toBeInTheDocument()
    expect(screen.getByText('角色提問次數')).toBeInTheDocument()
    ...
    expect(screen.getByText('監控端告警')).toBeInTheDocument()
    expect(screen.getAllByText('未告警').length).toBeGreaterThanOrEqual(1)
  })
